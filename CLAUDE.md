...
- **Label collisions — the same word names different controls.** Three separate axes, resolved one at a time:
  - **Rate vs. Length — RESOLVED (2026-07).** These are two distinct timing flavours, and a module carries **exactly one of them, or neither** — never both.
    ...
    - Open: Progression currently labels its bar-length step cadence "Rate," but it is neither a 1/32–1/1 note Rate nor a Chord/Drone Length. Decide whether Progression's step timing is a third named thing (e.g. a bar-length "Rate" that stays distinct) or folds into Length.
  - **"Repeat" collision — still open.** "Repeat" is the note-length reset window (Endless…4 bars) on Scale/Arp but the bar-length hold-repeat on Chord/Drone. (Note: Chord/Drone's "Length/Repeat" pair needs reconciling with the new Length model above.)
  - **"Octaves" vs "Octave" collision — still open.** "Octaves" (1–4 range span, Scale/Arp) reads almost identically to "Octave" (±2 transpose offset, Drone; also per-step in Progression). Rename to disambiguate (leaning this) vs. document the distinction.
