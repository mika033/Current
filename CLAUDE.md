...
timed `isRestoringState()` window. See architecture.md's "Messaging area
(help bar)" section.
...
keeps their timing) stays distinct in modules.md. The step grid has a
...
playhead (see "Transport and clocks" in architecture.md).
...
- **One accepted regression**, documented in modules.md/architecture.md: a
...
See `architecture.md`: component map, canvas-model (modules + connections) ownership and persistence, the graph-snapshot handoff to the audio thread, the engine's per-node graph execution, the module-catalogue extension recipe, theming, and the deferred items.
