...
- Build (and launch) before updating the docs, so the user can already test
  the change while the doc pass happens.
...
The basics and canvas skeleton from `docs/generative-midi-plugin-requirements.md` are implemented, plus the two I/O modules (MIDI In / Output) with per-module channel settings, a reworked Random generator (rate, root/scale override, note range), a new Scale generator (root/scale override, up/down, octaves, end-on, rate, repeat), and an LFO generator (root/scale override, shape, cycle length in bars, depth in octaves + scale steps, rate, phase) — all with full settings dialogs. Settings that recur across modules (root/scale override, rate, repeat, mode, octaves, gate) are shared: one option table + settings blob in `ModuleSettings.h`, one dialog-control helper pair per setting in `Canvas`, documented in the "Shared settings" section of `docs/modules.md`. The Arp is a modulator (not a generator) with full settings (mode, rate, octaves, gate, repeat); Shift has its settings too (amount ±36, scale Global/Off/named — degrees vs. chromatic), and the Delay — the first stateful time modulator — is in with rate, feedback, and a cumulative per-echo semitone shift. The global Quantize checkbox is gone; in its place Quantize is a timing modulator (rate grid + pair-based swing per the standards repo's `swing-timing.md`, LAM-style), pitch-snapping moved to a new Scale modulator (root/scale, default Global), and a Progression modulator walks a user-defined step list (degree I–VII + octave ±2 per step, 1–8 steps, bar-length rate) transposing everything through it. Two slow generators arrived after that: the Chord (a diatonic stack — degree I–VII, type Triad/7th/Sus2/Sus4/5th/6th, inversion — emitted on a bar-based Length/Repeat window) and the Drone (holds a voicing — root / root+5th / root+octave / triad, octave ±2 — on the same window model, defaulting to 4 bars, re-triggering immediately when its harmony changes mid-hold; bypasses Quantize and Delay by design). Their Length/Repeat pair joined the shared settings (the bar-length list, which gained a 16-bars entry). Every module in the palette now has a real settings dialog (`InlineDialog` grew same-row combo pairs, post-show add/remove, and non-closing utility buttons for the Progression's dynamic step rows). A **Humanize** modulator has since been added (the first module built from the "Planned — speced" catalogue): a final-stage performance-feel pass over the whole outgoing stream (played, generated, quantized, and delayed notes alike). Its window is Rate in the menu bar over a 3x2 grid of 0–100% dials laid out in signal-flow order — top row structured groove (Swing, Lay-back, Accent), bottom row random touch (Timing, Velocity, Length jitter). Swing reuses the Quantize pair-based model, factored into a shared `swungBoundaryQn`/`swingWarpQn` helper and applied as a continuous delay-only nudge (not a snap). All timing moves are delays (a real-time MIDI FX can't advance a note); the random amounts are a deterministic hash of song position, so the feel repeats every loop. Implemented as an isolated post-pass at the end of `Engine::process` (buffer `pendingHuman`, held-jitter map for length preservation, transport-stop balancing) — see docs/modules.md's Humanize entry and docs/architecture.md. Push-ahead (playing before the beat) is the one deferred piece: it needs host latency reporting, so only lay-back ships. A **Strum** modulator (the second module built from the "Planned — speced" catalogue) followed: a time modulator that spreads a chord's simultaneous notes out over a short window like a strummed guitar. Note-ons landing within a small fixed detection window are grouped into one chord (that window is the module's added latency, since the fan order can't be chosen until the whole chord is in), then released one after another one Spread gap apart (tempo-synced per-note gap, Off to an 1/8 note per consecutive note — see docs/modules.md for the current spec); Direction is the shared Mode (Up/Down/Up-Down/Random), Curve shapes the inter-note spacing (Even/Accelerate/Decelerate), Velocity tilts loudness ±100% across the fan, Jitter adds deterministic per-note looseness, and Repeat (the shared list) re-strums a held chord on the bar grid for comping. Delay-only like every timing module here; each note-off is delayed by the same amount as its note-on (a `strumHeld` map) so lengths and order hold and nothing hangs. Implemented as a post-pass just before Humanize (`strumGroup` accumulator, `pendingStrum` buffer, discard-on-stop) — see docs/modules.md's Strum entry and docs/architecture.md. A **Mirror** modulator (the third from the "Planned — speced" catalogue) then joined the pitch transformers: it inverts every note around a centre note — diatonically in scale degrees with a scale active (stays in key), chromatically in semitones with Scale = Off — then constrains the result to a Low/High register window, either dropping out-of-window notes (Bounds = Limit) or folding them back once across the nearest edge and clamping (Bounds = Mirror). Centre has an Off detent (no inversion, window only), and the Low/High dials push each other so the window can't invert (a new `ModuleWindow::setDialChangeCallback`/`setDialValue` pair). It rides `mapPitch` as the final pitch stage (`reflectAround`/`scaleStepsBetween` helpers) and is the one stage that can *drop* a note — Limit returns -1, and all three `mapPitch` call sites skip it, so the dropped note books no note-off and nothing hangs. A **Harmonizer** modulator (the fourth from the "Planned — speced" catalogue) then joined the pitch transformers: it stacks a chord on each played note — the Chord generator's voicing brain driven by live input rather than a fixed degree. The played note is the bass and passes through as usual; the extra voices (Type Triad/7th/Sus2/Sus4/5th/6th/Octave/Octave+5th, Inversion) are stacked above it — diatonic scale-degree stacking in its Root/Scale, or fixed chromatic intervals with Scale = Off — and each voice runs through the same `mapPitch` chain and Quantize/Delay as the played note. It is not a `mapPitch` stage because it changes the note *count*; instead it sits at the front of the host-note path (`emitHostNote`/`emitHostTone`/`harmVoicesRaw`), so like the Arp it harmonises played input only (and does nothing while an Arp swallows the host notes). Mode picks how simultaneous held notes interact: Add harmonises every held note, Replace keeps only the newest (monophonic, cutting the previous stack), Top harmonises only the highest held note and re-harmonises the next-highest when the top is released — bookkept over a new `harmHeld` set (velocity-carrying, unlike `held`) with `harmVoice`-tagged `activePass` entries, so the voices ride out on their played note's own note-off with no transport-stop or removal flush. That takes the palette to seventeen modules. **Port wiring then landed** (see the "Port wiring" section below): the fixed implicit chain and its per-type atomics are gone, the engine executes the user's wired graph per-node, the canvas has the full connect gesture (cable drag, selection, delete), connections persist with the project, and a fresh instance starts with MIDI In → Output wired. Where this paragraph's older narrative mentions fixed-chain behaviours (the Harmonizer acting on played input only, the Drone bypassing Quantize/Delay by design), the wiring section and docs/modules.md supersede it — routing is now the user's choice.

An **Audition Synth** and a **settings space** followed (both ported from / modelled on LAM). The synth (`AuditionSynth.h/.cpp`, LAM's voice verbatim: supersaw pluck, filter-envelope smack, delay→reverb on one Space dial, CC modulation, declick fade) voices the outgoing MIDI right after `Engine::process`, defaults ON in the Standalone / OFF in DAWs, and is absent from the AU MIDI-FX (no audio bus there). One deliberate divergence: the amp envelope always uses LAM's sustaining shape, because Current's note lengths are user-authored (Gate/Length/Drone holds) and must be honoured. The menu bar's theme switch became a **Settings** button that swaps canvas + palette for `SettingsView` (LAM's stacked-panel layout: Global panel with the Theme button, Audition Synth panel with toggle + five dials with live-readout labels). Two Standalone behaviour changes landed with this: the Standalone **persists no plugin state** (boots an empty canvas every launch; theme + manual tempo live in a `PropertiesFile` under Snorkel Audio, the LAM approach), and it answers **`isMidiEffect() == false`** — JUCE's `AudioProcessorPlayer` gives a MIDI-effect processor a zero-channel buffer and silences the device, which is why the synth was inaudible until that fix (details in docs/architecture.md's Build and test). A headless `current_audition_test` covers the whole path.

A **help bar** (the standards repo's messaging area, modelled on LAM's) now
runs along the editor's bottom edge, below the palette: touching any UI
element — menu bar, palette chips and filters, canvas gestures (select /
connect / add / delete), every `ModuleWindow` control including the
Progression and Rhythmize custom bodies, and the settings space — shows
"<Label>: <value> - <description>", transient at 3 s. Descriptions live in
`resources/help.json` (compiled in via `juce_add_binary_data`, loaded by
`HelpText`), keyed by control name with per-module overrides and per-option
entries; `MainView::showFeedback` is the single funnel (editor-level
`showFeedback` forwards to it), with restore suppression via the processor's
timed `isRestoringState()` window. See docs/architecture.md's "Messaging area
(help bar)" section.

The plugin builds and runs on Linux (VST3 + Standalone) and macOS (Universal VST3 + Standalone + an AU MIDI-FX `.component` that passes `auval`); a Windows script exists but is unverified, and the iPad target is not yet ported. See "Build instructions" for the per-OS scripts. An architecture/code review lives in `docs/code-review-2026-07.md`; the two transport findings it originally opened with — the engine ignoring host song position, and the Standalone having no transport so generators were silent — are now fixed, copying LAM's approach: every engine grid is re-derived each block from the host ppq position (no freewheeling counters — see "Transport and clocks" in `docs/architecture.md`), and the Standalone/playhead-less hosts get a synthesized internal transport with a Play toggle and Tempo stepper in the menu bar. The review's remaining findings are still open.
...
cells, dials for knob-friendly values; see `docs/architecture.md`) for all thirteen
...
arranger-style step row (see below and `docs/module-window.md`). `InlineDialog`
...
Settings live-apply + fine dial resolution (2026-07): every settings-window
edit is pushed to the engine immediately, so it is audible while the window is
still open — OK keeps the last push, Cancel/Esc/click-outside restores the
open-time snapshot. One shared path owns this (`Canvas::wireDialog` +
`ModuleWindow::onChanged`, with `ProgressionStepList::onChanged` forwarded
into it; the channel dialog repeats the plumbing inline since a channel isn't
a `ModuleSettings`), so a dialog now supplies only its controls and a
read-back lambda. Alongside it, every percent-style control became a
1%-resolution dial storing the real percent (Gate 1–100, Swing and the five
Humanize amounts 0–100, Strum Velocity ±100 and Jitter 0–100, Delay Feedback
0–90 — Quantize's Swing and Delay's Feedback had been stepped combos) and
Strum's Spread stores a percent of an 1/8-note per-note gap (0–100: 0 = Off,
50 = 1/16, 100 = 1/8; the dial reads ms at the current tempo in between, via
the processor's `getCurrentBpm`); the old
option-index-based `gateNames`/`swingNames`/`feedbackNames` tables are gone
and `paramsFor` converts percent→fraction once. Old dev saves load these
fields with odd small values once (pre-release rule, no migration).

A **Rhythmize** modulator (2026-07) is the eighteenth module: a rhythm-applying
modulator — the planned Euclidean's retrigger-the-held-set model, driven by a
hand-set 16-step on/off pattern instead of a formula. While the transport plays
it consumes its input like the Arp and, on each active step, retriggers every
currently-held pitch at its played velocity (a velocity-carrying `heldVel`
array in `NodeState`) for Rate × Gate; inactive steps are silent, and stopped
input passes through. The pattern index is the grid index mod 16, so the
default 1/16 rate makes the pattern exactly one bar, loop-invariant. Its
window is the first to combine a custom body with grid cells: `ModuleWindow`'s
`setCustomBody` now coexists with filled cells (body box above a
rows-as-needed grid box; body-only and grid-only windows are unchanged), so
the pattern rides a new `RhythmizeStepGrid` body — two rows of eight step
boxes cloned from LAM's `GateButton` styling (radius 3, black-alpha outline,
hover/press shading; off = `widgetBg`, on = `accent`) — above the shared Gate
dial, with Rate in the menu bar. Engine-side it is an Arp-shaped node
(`processRhythmize`: `runSteps` + `activeGen`/`releaseDueGates`, stop flush);
the smoke test covers pattern gating, velocity preservation, all-off silence,
and stop balance. The planned "Rhythm" module (a gate that masks notes but
keeps their timing) stays distinct in docs/modules.md. The step grid has a
**playhead**: LAM's idiom — an accent-alpha halo behind the playing box,
polled at 20 Hz with a changed-step repaint gate. It rides a new general
engine→UI clock surface (`Engine::uiSongQn`/`uiPlaying` atomics, processor
accessors `playheadQn()`/`transportPlaying()`): the playing step is computed
UI-side as `floor(qn/stepQn) mod 16` from the module's live Rate — no
per-node feedback, can't drift from the audio, reusable by any future
playhead (see "Transport and clocks" in docs/architecture.md).

## Port wiring (IMPLEMENTED 2026-07)

Port wiring — the product's core premise — is in. Modules are connected by
dragging a cable from an output port to an input port; the cables are the
signal flow, and the engine executes exactly the wired graph. The key facts a
future session needs:

- **Strict graph semantics.** Host MIDI enters only through MIDI In nodes and
  leaves only through Output nodes; an unwired module is silent, a patch with
  no Output makes no sound, fan-out duplicates, fan-in merges. A fresh
  instance starts with MIDI In → Output wired (the friendly default). Cycles,
  duplicates, and port mismatches are refused at connect time
  (`CurrentAudioProcessor::canConnect`, the single validity rule).
- **Every placed module is independent** — own settings, own runtime state.
  The fixed-chain era's "first instance wins" and "Arp/Harmonizer see played
  input only" are gone: wire a generator into a Harmonizer and it harmonises
  generated notes.
- **Engine architecture.** The message thread bakes the model into an
  immutable `GraphSnapshot` (EngineGraph.h) published via SpinLock'd
  shared_ptr swap (audio thread try-locks, never blocks); the engine runs the
  topologically-sorted nodes with per-node state keyed by module id. Rewiring
  (topologyVersion bump) cuts sounding notes and resets node state; settings
  edits keep notes ringing. Per-node no-hanging-notes contract: see Engine.h.
- **One accepted regression**, documented in docs/modules.md/architecture.md: a
  Drone wired through a Progression no longer re-triggers mid-hold on a step
  change (it picks the new degree up at its next window); a "held notes
  follow the step change" Progression feature could restore it.
...
- Engine smoke test: configure with `-DCURRENT_BUILD_TESTS=ON`, build, run `current_engine_test`. Run it after any engine change. The same flag adds `current_audition_test`, an end-to-end headless check that a wired generator makes the audition synth produce audio through the real `processBlock`; run it after audition-synth or processBlock changes.
- Testing policy: keep running the existing smoke tests as above, but do not add smoke-test coverage for new modules or features unless the user asks for it.
- macOS: `./build_and_run_mac.sh` builds a Universal (arm64 + x86_64) VST3 + Standalone + AU into `build-mac/`, installs the VST3 into `~/Library/Audio/Plug-Ins/VST3` and the AU `.component` into `~/Library/Audio/Plug-Ins/Components`, runs `auval -v aumi Curr Snrk` (diagnostic only — never aborts the launch), and opens the Standalone. The AU is the only format Logic/GarageBand accept in their MIDI-FX slot; it passes `auval` because `makeBusesProperties` sheds its audio buses for the AU wrapper (see the AU note in `CMakeLists.txt`). Self-contained: configures `build-mac/` if missing, so no separate `generate_xcode.sh` is needed for a dev build. This script is the only mac/Windows path verified so far.
- Windows: `build_and_run_win.bat` builds the VST3 + Standalone into `build-win/`, installs the VST3 into `%CommonProgramFiles%\VST3` (best-effort — warns without an elevated shell rather than aborting), and launches the Standalone. No AU (macOS-only). Ported from Little Arp Monster but not yet run on a Windows box — treat as unverified until it is.
- Not yet ported from Little Arp Monster: `run_mac.sh` (launch-only), `generate_xcode.sh`, `release_*`, and the iPad target.

## Architecture

See `docs/architecture.md`: component map, canvas-model (modules + connections) ownership and persistence, the graph-snapshot handoff to the audio thread, the engine's per-node graph execution, the module-catalogue extension recipe, theming, and the deferred items.

## Module reference

See `docs/modules.md`: manual-style docs for the implemented modules plus the planned catalogue, tiered into speced (codable as-is) and idea-only (open questions listed per module). Keep it updated as modules gain real settings or specs firm up.
...
of the steps; the dialog reads it back with `getSteps()` on every live-apply push. A progression
