...
// Phase 2 processor. The module layout lives here (not in the editor) so it
// survives the editor being closed and reopened and is persisted in the DAW
// project state. processBlock drives the fixed-default Engine (there is no user
// wiring yet, so the modules run as an implicit chain — see Engine.h).
//
// Threading: the module list itself is touched only from the message thread
// (editor + canvas); processBlock never reads the vector. What the audio thread
// does read is the lock-free presence snapshot (the four atomic bools below),
// republished by refreshEngineConfig() on every model change. When connections
// land and processBlock has to consume the actual graph, this snapshot handoff
// must grow into a full message->audio graph publish, not a lock on the model.
